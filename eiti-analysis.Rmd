...
```{r message=FALSE}
pacman::p_load(tidyverse, ggthemes, nrgiR)
...
drc_eiti %>% 
  ...
  ungroup() %>% 
    ...
    labs(title ="Total annual contributions according to EITI summary data", x = "year", y = "Cumulative payments - USD")
```
...
drc_eiti %>% 
  ...
  summarise(total_pay = sum(total_pay, na.rm = T)) %>% 
  ungroup() %>% 
  ggplot(data = ., mapping = aes(x = fct_reorder(company_name, -total_pay), y = total_pay)) +
  geom_col() +
  coord_flip() +
  theme_economist() +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 30)) +
...
drc_eiti %>% 
  ...
  labs(title ="Top mining taxpayers's annual contributions according to EITI summary data", x = "year", y = "Cumulative payments - USD") +
  theme(
  )
```
...
drc_eiti %>% 
  ...
  summarise(total_pay = sum(total_pay, na.rm = T)) %>% 
  ungroup() %>% 
  ggplot(aes(x = fct_reorder(gfs_description, -total_pay), y = total_pay)) +
  geom_col() +
  coord_flip() +
  theme_economist() +
...
drc_eiti %>% 
  ...
  summarise(total_pay = sum(total_pay, na.rm = T)) %>% 
  ungroup() %>% 
  ggplot(aes(x = " ", y = total_pay, fill = fct_reorder(gfs_description, -total_pay))) +
...
drc_eiti %>% 
  ...
  nrgi_theme() +
  theme(axis.text.x = element_text(angle = 90)) +
  labs(title = str_wrap("Top mining taxpayers's annual contributions according to EITI summary data"), x = "year", y = str_wrap("Cumulative payments - USD"))
```
...
drc_eiti %>% 
  ...
  coord_flip() +
  scale_x_discrete(labels = function(x) str_wrap(x, width = 30)) +
  labs(title ="Top mining taxpayers's total contributions by main gfs description according to EITI summary data", y = "Cumulative payments - USD")
```
...
6/ Meltalko: exceptionnally large income tax payment in 2016
